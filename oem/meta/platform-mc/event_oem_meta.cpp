    ...
    const std::map<std::string, MctpEndpoint>& configurations)
{
    ...
    static constexpr auto errorSeverityDetail = std::to_array(
        {"Correctable Error", "Deferred Error",
         "Uncorrected Recoverable Error", "Uncorrected Thread Fatal Error",
         "Uncorrected System Fatal Error"}
    );

    static constexpr auto machineCheckBank = std::to_array({
        "LS", "IF", "L2", "DE", "RAZ", "EX", "FP", "L3", "L3", "L3",
        "L3", "L3", "L3", "L3", "L3", "MP5", "PB", "PCS_GMI", "PCS_GMI",
        "KPX_GMI", "KPX_GMI", "UMC/PB/RAZ", "UMC/PCIE/RAZ", "CS/RAZ",
        "CS/RAZ", "NBIO/SHUB/RAZ", "PCIE/SATA/RAZ", "PCIE/NBIF/RAZ",
        "PIE/PSP/KPX_WALF/NBIF/USB/RAZ", "SMU/MPDMA/RAZ", "PCS_XGMI/RAZ",
        "KPX_SERDES/RAZ"}
    );
    ...
    switch (recordType)
    {
        ...
        {
            // Currently, we only have one standard SEL needed to support on YV4.
            if (eventData[9] == 0x2B) {
                errorLog = "POST_ERROR:";
        ...
        case RecordType::UNIFIED_BIOS_SEL:
        {
            switch (errorType)
            {
                ...
                case UnifiedError::UNIFIED_MCA_ERR:
                {
                    ...
                    errorLog = "GeneralInfo: MCAErr(0x" +
                               to_hex_string(generalInfo) +
                               "), MCA Severity: " +
                               errorSeverityDetail[mcaSeverity] +
                ...
                case UnifiedError::UNIFIED_MCA_ERR_EXT:
                {
                    ...
                    errorLog = "GeneralInfo: MCAErrExt(0x" +
                               to_hex_string(generalInfo) +
                               "), MCA Severity: " +
                               errorSeverityDetail[mcaSeverity] +
                               ", CPU Number: " + std::to_string(cpuNumber) +
                ...
                default:
                {
                    errorLog = "Undefined Error Type(0x" +
